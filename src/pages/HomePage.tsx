...
import { Suspense, useRef, useLayoutEffect, useState, useCallback, useEffect, useMemo, memo } from 'react';
...
const TextPhrase2: FC<{ scrollPercentage: number }> = memo(({ scrollPercentage }) => {
  // Función para calcular opacidad basada en rango de scroll
  const calculateOpacity = useCallback((start: number, end: number): number => {
...
const HomePage: FC<HomePageProps> = ({ 
  ...
  // 🎯 RESPONSIVE HOOKS
  const { isMobile, isTablet, performanceConfig, prefersReducedMotion } = useResponsive();
  ...
  const responsiveConfig = useMemo(() => {
    return {
      ...
      animations: {
        duration: prefersReducedMotion ? 0.01 : performanceConfig.animations.duration,
    ...
    const detectScrollIssues = () => {
      ...
      if (scrollElement) {
        ...
        if (!hasHeight || !hasScrollableHeight) {
          console.warn('⚠️ PROBLEMA DETECTADO: Elemento de scroll sin dimensiones adecuadas');
        }
      }
    };
  ...
  useEffect(() => {
    ...
    console.log('🎵 Audio ambiente configurado, esperando scroll para iniciar...');
  ...
  useEffect(() => {
    ...
    return () => {
      if (transitionAudioRef.current) {
        ...
        console.log('🎵 Manteniendo audio de transición activo durante cambio de página');
    ...
    if (scrollPercentage > 0) {
      const startAmbientSound = async () => {
        try {
          ...
          console.log('✅ Sonido ambiente iniciado con scroll (ScrollTrigger detectado)');
        } catch (error) {
  ...
  const triggerPortalTransition = useCallback(() => {
    ...
    if (transitionAudioRef.current) {
      ...
      const playTransitionSound = async () => {
        try {
          ...
          console.log('⚠️ No se pudo reproducir el sonido de transición (posible política de autoplay):', error);
    ...
    const portalTimeline = gsap.timeline({
      ease: "power3.out",
      onComplete: () => {
        console.log('🎯 Portal transition complete - Timeline terminado correctamente');
      }
    });
    ...
    portalTimeline
      ...
      .to(canvas, {
        ...
        ease: "power2.in"
      }, 1.5) // Sincronizado para terminar en 2s
      ...
      .to(canvas, {
        ...
        ease: "power3.out"
      }, 1.7); // Timing final ajustado a 2s
  }, [navigate]);
  ...
  }, [renderTrail, responsiveConfig.mouseTrail.updateInterval, responsiveConfig.mouseTrail.maxPoints]);
  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
          ...
          if (target && target.style.cursor && target.style.cursor !== 'auto' && target.style.cursor !== 'pointer') {
            console.log('🎯 Interceptado cambio de cursor:', target.style.cursor);
            forceCursor();
          }
        }
      });
    ...
    document.addEventListener('mousemove', mouseEventHandler, { passive: true });
  ...
  useLayoutEffect(() => {
    ...
    const setupScrollTrigger = (attempt = 1) => {
      ...
      if (!checkReferences()) {
        if (attempt < 10) { // Aumentado de 5 a 10 intentos
      ...
      const timeline = gsap.timeline({
        scrollTrigger: {
          ...
          onUpdate: (self) => {
            ...
            const effectiveProgress = isEmbedded ? Math.min(progress, maxScrollPercentage) : progress;
            
            // 🚀 THROTTLING RESPONSIVO: Intervalos adaptativos según dispositivo
            const throttleInterval = responsiveConfig.animations.scrollThrottleInterval;
            if (Math.abs(effectiveProgress - scrollPercentage) >= throttleInterval) {
      ...
      if (!isEmbedded) {
        ScrollTrigger.create({
          ...
          onUpdate: (self) => {
            ...
            if (progress >= 68 && progress < 70 && !glitchTriggeredRef.current) {
            ...
            if (progress >= 70 && !portalTriggeredRef.current && !isTransitioning) {
        ...
        ScrollTrigger.create({
          ...
          onUpdate: (self) => {
            ...
            if (clampedProgress >= 60 && clampedProgress < 65 && !glitchTriggeredRef.current && maxScrollPercentage > 65) {
              glitchTriggeredRef.current = true;
              setIsDigitalGlitch(true);
      ...
      console.log(`✅ ScrollTrigger configurado exitosamente. Triggers activos: ${activeScrollTriggers.length}`);
    };
    ...
    const timeoutId = setTimeout(() => {
      ...
      if (!isCanvasReady) {
        console.log('🕒 Timeout fallback: ejecutando setup sin esperar Canvas ready');
  ...
  }, [triggerPortalTransition, responsiveConfig.animations.scrollThrottleInterval, scrollContainer, isEmbedded, maxScrollPercentage]); // 🎯 Dependencias corregidas
  ...
  return (
    <div ref={mainRef} className="homepage-container">
      ...
      <div 
        ref={canvasRef}
        className={`canvas-container ${isTransitioning ? 'transitioning' : ''} ${isDigitalGlitch ? 'digital-glitch' : ''}`}
      >
        <Canvas
          gl={{ 
