...
const AnimatedTextPhrase1: FC<AnimatedTextPhrase1Props> = memo(({ scrollPercentage }) => {
  const groupRef = useRef<THREE.Group>(null!);

  // Función para calcular opacidad basada en rango de scroll
  const calculateOpacity = (start: number, end: number): number => {
  ...
  const opacidades = useMemo(() => {
    return {
      line1: {
        ...
        opacity: scrollPercentage < 12 ? 0 : Math.min(1, calculateOpacity(12, 15))
      },
      line2: {
