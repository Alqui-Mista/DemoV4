...
const selectorLines = css.split('{').map(s => s.split('\n').slice(0,1)[0]).map(s => s.trim()).filter(Boolean);
...
selectorLines.forEach(line => {
  ...
  line.split(',').forEach(part => {
    ...
    const cleaned = token.replace(/::?[^\s\.:#\[]*/g, '').replace(/\[.*?\]/g,'').replace(/>+/g,'').replace(/\s+/g,'').replace(/,$/,'');
...
function usedSelector(sel){
  ...
  for (const fp of files){
    ...
    if (isClass){
      ...
      const cre = new RegExp('class(Name)?\\s*=\\s*["\'`][^"\'`]*\\b'+escapeReg(name)+'\\b[^"\'`]*["\'`]','m');
      if (cre.test(content)) return true;
      // querySelector('.token') or getElementsByClassName('token')
      const cre2 = new RegExp("(querySelectorAll|querySelector)\\s*\\(\\s*['\"]\\."+escapeReg(name)+"['\"]\\s*\\)");
      if (cre2.test(content)) return true;
      const cre3 = new RegExp("getElementsByClassName\\s*\\(\\s*['\"]"+escapeReg(name)+"['\"]\\s*\\)");
      ...
      // id
      const cre = new RegExp('id\\s*=\\s*["\'`][^"\'`]*\\b'+escapeReg(name)+'\\b[^"\'`]*["\'`]','m');
      if (cre.test(content)) return true;
      const cre2 = new RegExp("(querySelectorAll|querySelector)\\s*\\(\\s*['\"]#"+escapeReg(name)+"['\"]\\s*\\)");
      if (cre2.test(content)) return true;
      const cre3 = new RegExp("getElementById\\s*\\(\\s*['\"]"+escapeReg(name)+"['\"]\\s*\\)");
      if (cre3.test(content)) return true;
      const cre4 = new RegExp('\\b'+escapeReg(name)+'\\b');
      if (cre4.test(content)) return true;
    }
...
console.log(JSON.stringify({unused, totalSelectors: results.total, checkedFiles: files.length}, null, 2));
